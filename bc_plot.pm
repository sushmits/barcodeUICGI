...
my $output = "eps";
my $gnuplot_path = "gnuplot";
my $fig2dev_path = "fig2dev";
my $debug_seefig_unmod = 0;
my $png_transparent = 1;
my $verbose = 0;
...
my $gnuplot_version = `$gnuplot_path --version`;
$gnuplot_version =~ /gnuplot ([\d\.]+)/;
$gnuplot_version = $1;
my $gnuplot_uses_offset = 1;
 $gnuplot_uses_offset = 0 if ($gnuplot_version <= 4.0);
# For gnplot 5.x use "dashtype" or "dt", else use "linetype" or "lt".
my $linetype = ($gnuplot_version < 5.0) ? "lt" : "dt";

# support for clusters and stacked
my $stacked = 0;
my $stacked_absolute = 0;
my $stackcount = 1;
my $clustercount = 1;
my $plotcount = 1; # multi datasets to cycle colors through
my $dataset = 0;
my $table = 0;
...
#            $fillcolor[3]=$fig_red;
            $fillcolor[0]=$fig_light_green;
            $fillcolor[1]=$fig_red;
            $fillcolor[2]=$fig_light_blue;
            $fillcolor[3]=$fig_black;
        } elsif ($colorcount == 5) {
